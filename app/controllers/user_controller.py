...
class UserController:
    ...
    def update_user(self, user_id: int, user: User):
        try:
            conn = get_db_connection()
            cursor = conn.cursor()
            cursor.execute("UPDATE usuarios SET nombre=%s, apellido=%s, cedula=%s, edad=%s, usuario=%s, contrasena=%s WHERE id=%s", 
                           (user.nombre, user.apellido, user.cedula, user.edad, user.usuario, user.contrasena, user_id))
            conn.commit()
            conn.close()
